...
describe "SSPE::Parser" do
  ...
  describe "#parse" do
    ...
    it "raises an SSPE::FormatError if the file has less than 3 lines" do
      ...
      expected_message = "Input file should have 3 lines. It has 2."
      expect { parser.parse }.to raise_error(SSPE::FormatError, expected_message)
    end

    it "raises an SSPE::FormatError if the file has more than 3 lines" do
      @filename = './spec/more_lines.txt'
      expected_message = "Input file should have 3 lines. It has 4."
      expect { parser.parse }.to raise_error(SSPE::FormatError, expected_message)
    end

    it "raises an SSPE::FormatError if a line is incomplete" do
      @filename = './spec/incomplete_line.txt'
      expected_message = "All lines in input file should have a length which is a multiple of 4, length of line 3 is wrong."
      expect { parser.parse }.to raise_error(SSPE::FormatError, expected_message)
    end

    it "raises an SSPE::FormatError if not all lines are the same length" do
      @filename = './spec/different_lengths.txt'
      expected_message = "All lines in input file should be the same length. Lines have lengths of 40, 36, 32 respectively."
      expect { parser.parse }.to raise_error(SSPE::FormatError, expected_message)
    end

    it "raises an SSPE::InputError if an unknown digit is parsed" do
      @filename = './spec/unknown_digit.txt'
      expected_message = "Unrecognized digit in input file."
      expect { parser.parse }.to raise_error(SSPE::InputError, expected_message)
    end
  end
end
